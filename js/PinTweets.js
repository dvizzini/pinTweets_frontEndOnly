...
function highlight(marker) {
	...
	for (i=0;i<global.markers.length;i++) {
		global.markers[i].setIcon('images/blue_Marker' + String.fromCharCode(i+65) +'.png');
		maxZIndex = Math.max(maxZIndex,global.markers[i].getZIndex);
	}
...
function geocodeTweets(map,data) {
	...
	var geocode = function(n) { //recursive
		if (n>0) {
			...
			} else if (searched.name.indexOf(userName) != -1) {
